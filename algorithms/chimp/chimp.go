...
func CompressFloat(dst []byte, src []float64) []byte {
	...
	bs.AppendBits(math.Float64bits(src[0]), 64)
	lastLeading := 0
	lastValue := src[0]
	for i := 1; i < len(src); i++ {
		xor := math.Float64bits(src[i]) ^ math.Float64bits(lastValue)
		leading := bits.LeadingZeros64(xor)
		if leading > 15 {
			leading = 15
		}
...
func DecompressFloat(dst []float64, src []byte) ([]float64, error) {
	...
	for i := uint64(1); i < length; i++ {
		...
		dst = append(dst, math.Float64frombits(curValue))
		leading := bits.LeadingZeros64(curValue ^ lastValue)
		if leading > 15 {
			leading = 15
		}
