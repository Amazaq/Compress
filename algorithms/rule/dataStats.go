...
func AnalyzeTimeSeries(src []float64) *TimeSeriesStats {
	...
	// 基本统计
	computeBasicStats(src, stats)

	// 数值特性
	computeValueProperties(src, stats)

	// 差分统计
	if len(src) > 1 {
		stats.DiffStats = computeDifferenceStats(src, 1)
		if len(src) > 2 {
			stats.SecondDiffStats = computeDifferenceStats(src, 2)
			stats.Smoothness = stats.SecondDiffStats.StdDev
		}
	}

	// 时序特征
	computeTimeSeriesFeatures(src, stats)

	// 游程统计
	stats.RunLength = computeRunLengthStats(src)

	// 位级统计
	stats.BitStats = computeBitLevelStats(src)

	// 分布特征
	stats.Entropy = computeEntropy(src)
